...
<body>
    ...
    <div class="card">
        ...
        <div class="info-box">
            <strong>PIC Digantikan:</strong> {{ $dailyJob->member->nama ?? 'Tidak diketahui' }}<br>
            <strong>NIK Pengganti:</strong> {{ session('replacement_nik') }}
        </div>
        ...
        <a href="{{ route('replacements.finish') }}" class="btn-finish" style="display: block; text-align: center; text-decoration: none; box-sizing: border-box;">Selesai Scan</a>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function onScanSuccess(decodedText, decodedResult) {
                ...
                $.ajax({
                    url: "{{ route('replacements.storeScan') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                    ...
                    success: function(res) {
                        if (res.success) {
                            ...
                            // Hembuskan notifikasi sebentar lalu sembunyikan
                            setTimeout(() => {
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 401) {
                            window.location.href = "{{ route('replacements.start') }}";
                        }
