...
<script>
    ...
    const scans = @json($scansForJs ?? []);
    const costs = @json($costsForJs ?? []);
    const powers = @json($powersForJs ?? []);
    const penanganans = @json($penanganansForJs ?? []);

    const memberHours = {{ $memberHours ?? 0 }};
    const reportMembers = {{ $reportMembers ?? 0 }};
    const powerTotal = {{ $powerTotal ?? 0 }};
